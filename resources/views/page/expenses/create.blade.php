      ...
      <nav class="col-1 vh-100 p-3" style="position: sticky; top:0;">
        ...
        <ul class="nav flex-column">
          ...
          <li class="nav-item"><a class="nav-link btn btn-primary" href="" style="background-color: #0d6efd">
            <img src="{{ asset('img/spending.png') }}" alt="" style="width: 25px;">Thêm</a></li>
    ...
    <ul class="dropdown-menu text-small shadow w-100" aria-labelledby="userDropdown">
      ...
      <main class="col-11 p-4">
        <div class="d-flex justify-content-between align-items-center">
         ...
         <button type="button" class="btn btn-primary">Chi tiêu</button>
          <a href="{{ route('page.budgets.index') }}" class="btn btn-outline-primary text-decoration-none">Đặt mục tiêu</a>
        </div>
      </div>
      <hr>
        ...
        <form action="{{ route('page.expenses.store') }}" method="POST">
          @csrf
          <div class="row p-4">
            <!-- Cột trái -->
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="date" class="form-control" id="spend_date" name="spend_date" 
                       value="{{ old('spend_date', date('Y-m-d')) }}" required>
              </div>
              <div class="mb-3">
                <label for="amount" class="form-label">Số tiền chi tiêu</label>
                <div class="input-group">
                  <input type="number" class="form-control" id="amount" name="amount" 
                         placeholder="Nhập số tiền..." value="{{ old('amount') }}" min="1" required>
                  <span class="input-group-text">VNĐ</span>
                </div>
              </div>
              <div class="mb-3">
                <label for="note" class="form-label">Ghi chú</label>
                <input type="text" class="form-control" id="note" name="note" placeholder="Nhập ghi chú..." value="{{ old('note') }}">
              </div>
            </div>
            ...
            <div class="col-md-6">
              ...
              <div class="row g-2">
                @foreach ($categories as $category)
                  <div class="col-4">
                    <button type="button" class="btn btn-outline-primary w-100 category-button" data-id="{{ $category->id }}">
                      {{ $category->name }}
                    </button>
                  </div>
                @endforeach
          ...
          <div class="text-center mt-4">
            <button type="submit" class="btn btn-success px-5">Thêm khoản chi</button>
          </div>
        </form>
      </main>
    </div>
